...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const handleSaveProduct = (e: React.FormEvent) => {
    ...
    // Filter out any blank image URLs
    let cleanImages = (formData.images || []).filter(img => img && img.trim().length > 0);
    if (cleanImages.length === 0) {
      cleanImages = ['https://images.unsplash.com/photo-1539109136881-3be0616acf4b'];
    }

    // Sync primary image to first color variant if present
    let cleanColors = (formData.colors || []).map((c, i) => {
      if (i === 0 || !c.imageUrl || !c.imageUrl.trim()) {
        return { ...c, imageUrl: cleanImages[0] };
      }
      return c;
    });

    if (cleanColors.length === 0) {
      cleanColors = [{ name: 'Noir', hex: '#1e1b19', imageUrl: cleanImages[0] }];
    }
  ...
  const handleUpdateImageUrl = (index: number, url: string) => {
    setFormData((prev) => {
      ...
      if (index === 0 && prev.colors && prev.colors.length > 0) {
        updatedColors = prev.colors.map((c, i) => i === 0 ? { ...c, imageUrl: url } : c);
      }
      return { ...prev, images: imgs, colors: updatedColors };
    });
  };
  ...
  const handleAddColorField = () => {
    setFormData((prev) => ({
      ...
      colors: [
        ...(prev.colors || []),
        { name: 'Color', nameAr: 'لون جديد', hex: '#111111', imageUrl: prev.images?.[0] || '' },
      ],
    }));
  };
  ...
  const handleUpdateColor = (index: number, key: keyof ProductColor, value: string) => {
    setFormData((prev) => {
      ...
      if (key === 'imageUrl' && index === 0 && value) {
        updatedImages = [...(prev.images || [])];
        updatedImages[0] = value;
      }
      return { ...prev, colors: updatedColors, images: updatedImages };
    });
  };
  ...
  const handleFileUploadForColor = async (index: number, file: File) => {
    ...
    if (compressed) {
      handleUpdateColor(index, 'imageUrl', compressed);
    }
  };
  ...
  const handleFileUploadForMainImage = async (index: number, file: File) => {
    ...
    const compressed = await compressImageFile(file, 800, 0.72);
    if (compressed) {
      handleUpdateImageUrl(index, compressed);
      if (index === 0 && formData.colors && formData.colors.length > 0) {
        handleUpdateColor(0, 'imageUrl', compressed);
      }
    }
  };
  ...
  const handleFileUploadForCategoryImage = async (file: File) => {
    ...
    const compressed = await compressImageFile(file, 800, 0.72);
    if (compressed) {
      setCategoryForm((prev) => ({ ...prev, imageUrl: compressed }));
    }
  };
        ...
        {activeTab === 'settings' && (
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-[#c4c7c7]/30 shadow-xs space-y-6 fade-in-up max-w-4xl mx-auto">
            ...
            <form onSubmit={handleSaveSettings} className="space-y-6">
              ...
              {/* Announcement Bar */}
              <div className="space-y-4 p-5 bg-[#f9f9f9] rounded-2xl border border-[#c4c7c7]/30 shadow-2xs">
                <div className="flex items-center justify-between border-b border-[#c4c7c7]/20 pb-3">
                  <h3 className="font-label-caps text-[14px] font-bold text-[#000000] flex items-center gap-2">
                    <span className="material-symbols-outlined text-[20px] text-[#000000]">campaign</span>
                    <span>{language === 'ar' ? 'التحكم بالشريط الإعلاني العلوي (Top Ticker Bar)' : 'Top Announcement Ticker Bar Controls'}</span>
                  </h3>
                  <label className="flex items-center gap-2 cursor-pointer bg-white px-3 py-1.5 rounded-lg border border-[#c4c7c7]/50 shadow-2xs">
                    <input
                      type="checkbox"
                      checked={settingsForm.showAnnouncementBar !== false}
                      onChange={(e) => setSettingsForm({ ...settingsForm, showAnnouncementBar: e.target.checked })}
                      className="w-4 h-4 accent-[#000000] rounded cursor-pointer"
                    />
                    <span className="text-[12px] font-bold text-[#000000]">
                      {language === 'ar' ? 'تفعيل الشريط' : 'Enable Ticker Bar'}
                    </span>
                  </label>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[12px] font-label-caps font-bold text-[#000000] mb-1">
                      {language === 'ar' ? 'الإعلان الرئيسي (بالعربية)' : 'Main Announcement (Arabic)'}
                    </label>
                    <input
                      type="text"
                      ...
                      placeholder="مثال: شحن مجاني لجميع المحافظات | خصم 10% كود TOUZA10"
                      className="w-full border border-[#c4c7c7] rounded-xl py-2 px-3 text-[14px] bg-white focus:outline-none focus:border-[#000000]"
                    />
                  </div>
                  <div>
                    <label className="block text-[12px] font-label-caps font-bold text-[#000000] mb-1">
                      {language === 'ar' ? 'الإعلان الرئيسي (بالإنجليزية)' : 'Main Announcement (English)'}
                    </label>
                    <input
                      type="text"
                      ...
                      placeholder="e.g. Free Express Shipping Nationwide | Code TOUZA10"
                      className="w-full border border-[#c4c7c7] rounded-xl py-2 px-3 text-[14px] bg-white focus:outline-none focus:border-[#000000]"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-[12px] font-label-caps font-bold text-[#000000] mb-1">
                      {language === 'ar' ? 'النص الثانوي التكراري (بالعربية)' : 'Secondary Text (Arabic)'}
                    </label>
                    <input
                      type="text"
                      value={settingsForm.announcementSecondaryAr || ''}
                      onChange={(e) =>
                        setSettingsForm({ ...settingsForm, announcementSecondaryAr: e.target.value })
                      }
                      placeholder="مثال: توزا لأزياء الرجال - TOUZA MEN'S WEAR"
                      className="w-full border border-[#c4c7c7] rounded-xl py-2 px-3 text-[14px] bg-white focus:outline-none focus:border-[#000000]"
                    />
                  </div>
                  <div>
                    <label className="block text-[12px] font-label-caps font-bold text-[#000000] mb-1">
                      {language === 'ar' ? 'النص الثانوي التكراري (بالإنجليزية)' : 'Secondary Text (English)'}
                    </label>
                    <input
                      type="text"
                      value={settingsForm.announcementSecondaryEn || ''}
                      onChange={(e) =>
                        setSettingsForm({ ...settingsForm, announcementSecondaryEn: e.target.value })
                      }
                      placeholder="e.g. TOUZA MEN'S WEAR - PREMIUM CASUALS"
                      className="w-full border border-[#c4c7c7] rounded-xl py-2 px-3 text-[14px] bg-white focus:outline-none focus:border-[#000000]"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-[12px] font-label-caps font-bold text-[#000000] mb-1">
                      {language === 'ar' ? 'لون خلفية الشريط' : 'Background Color'}
                    </label>
                    <div className="flex items-center gap-2">
                      <input
                        type="color"
                        value={settingsForm.announcementBgColor || '#111111'}
                        onChange={(e) =>
                          setSettingsForm({ ...settingsForm, announcementBgColor: e.target.value })
                        }
                        className="w-10 h-10 rounded-lg border border-[#c4c7c7] cursor-pointer p-0.5 bg-white"
                      />
                      <input
                        type="text"
                        value={settingsForm.announcementBgColor || '#111111'}
                        ...
                        className="w-full border border-[#c4c7c7] rounded-xl py-2 px-3 text-[14px] bg-white font-mono"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="block text-[12px] font-label-caps font-bold text-[#000000] mb-1">
                      {language === 'ar' ? 'لون الخط والرموز' : 'Text & Icon Color'}
                    </label>
                    <div className="flex items-center gap-2">
                      <input
                        type="color"
                        value={settingsForm.announcementTextColor || '#e2c792'}
                        onChange={(e) =>
                          setSettingsForm({ ...settingsForm, announcementTextColor: e.target.value })
                        }
                        className="w-10 h-10 rounded-lg border border-[#c4c7c7] cursor-pointer p-0.5 bg-white"
                      />
                      <input
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-[12px] font-label-caps font-bold text-[#000000] mb-1">
                      {language === 'ar' ? 'سرعة حركة الشريط' : 'Ticker Movement Speed'}
                    </label>
                    <select
                      value={settingsForm.announcementSpeed || 'normal'}
                      onChange={(e) =>
                        setSettingsForm({
                          ...settingsForm,
                          announcementSpeed: e.target.value as 'slow' | 'normal' | 'fast',
                        })
                      }
                      className="w-full border border-[#c4c7c7] rounded-xl py-2 px-3 text-[14px] bg-white focus:outline-none focus:border-[#000000]"
                    >
                      <option value="slow">{language === 'ar' ? 'بطيء وهادئ (Slow - 36s)' : 'Slow (36s)'}</option>
                      <option value="normal">{language === 'ar' ? 'متوسط قياسي (Normal - 24s)' : 'Normal (24s)'}</option>
                      <option value="fast">{language === 'ar' ? 'سريع وملفت (Fast - 14s)' : 'Fast (14s)'}</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-[#f9f9f9] rounded-xl border border-[#c4c7c7]/20 space-y-3">
                <label className="block text-[12px] font-label-caps font-bold text-[#000000]">
                  {language === 'ar' ? 'رابط فيديو / صورة الهيدر الرئيسي (Hero Video or Image)' : 'Hero Video or Image URL'}
                </label>
                <p className="text-[11px] text-[#555555]">
                  {language === 'ar'
                    ? 'اختر من الفيديوهات الفاخرة المدمجة المتاحة، أو أدخل رابط فيديو MP4/صورة مخصصة:'
                    : 'Choose from integrated luxury video templates or enter custom MP4/image URL:'}
                </p>

                {/* Quick Selection Buttons */}
                <div className="flex flex-wrap gap-2 pt-1">
                  {[
                    { label: language === 'ar' ? 'فيديو 1 (الأساسي)' : 'Video 1 (Hero Main)', path: '/hero-video.mp4' },
                    ...
                    <button
                      key={v.path}
                      type="button"
                      onClick={() => setSettingsForm({ ...settingsForm, heroImageUrl: v.path })}
                      className={`px-3 py-1.5 rounded-lg text-[12px] font-label-caps font-medium transition-all cursor-pointer ${
                        settingsForm.heroImageUrl === v.path
                          ? 'bg-black text-white font-bold shadow-xs'
                          : 'bg-white text-black border border-[#cccccc] hover:bg-[#eaeaea]'
                      }`}
                    >
                      {v.label}
                    </button>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSettingsForm({ ...settingsForm, heroImageUrl: e.target.value })
                  }
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                      <input
                        ...
                        placeholder="أدخل رابط الصورة أو اكتب hide لإخفائها"
                      />
                        ...
                        <input
                          ...
                          onChange={async (e) => {
                            ...
                            if (file) {
                              const compressed = await compressImageFile(file, 800, 0.72);
                              if (compressed) {
                                setSettingsForm((prev) => ({ ...prev, philosophyImageUrl: compressed }));
                              }
                            }
                          }}
                        />
                      ...
                      <button
                        type="button"
                        onClick={() => setSettingsForm((prev) => ({ ...prev, philosophyImageUrl: 'hide' }))}
                        className="px-3 py-2 bg-red-100 text-red-700 hover:bg-red-200 text-[12px] font-bold rounded-xl transition-colors shrink-0"
                      >
                        {language === 'ar' ? 'إخفاء الصورة' : 'Hide Image'}
                      </button>
                    </div>
