...
export const ProductDetail: React.FC<ProductDetailProps> = ({
  ...
  const handleSelectColor = (colorObj: { name: string; nameAr?: string; hex: string; imageUrl?: string }, index = 0) => {
    setSelectedColor(colorObj.name);
    if (index === 0 || colorObj.imageUrl === product.images?.[0]) {
      setActiveColorImage(null);
    } else if (colorObj.imageUrl) {
  ...
  // Gather all unique images, placing main product images first
  const colorImagesList = (product.colors || [])
    .map((c) => c.imageUrl)
    .filter((img): img is string => Boolean(img));
  const galleryImages = Array.from(new Set([...(product.images || []), ...colorImagesList]));
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 mb-20">
        ...
        <div className="lg:col-span-7 flex flex-col-reverse lg:flex-row gap-4 items-start">
          ...
          {galleryImages.length > 1 && (
            <div className="flex lg:flex-col gap-3 overflow-x-auto lg:overflow-visible hide-scrollbar w-full lg:w-20 shrink-0">
              {galleryImages.map((img, idx) => (
                ...
                >
                  <img
                    src={img || '/images/touza_green_shirt.jpg'}
                    alt={`${displayName} view ${idx + 1}`}
                    onError={(e) => {
                      handleProductImageError(e, product.id);
                    }}
          ...
          <div className="w-full h-[360px] sm:h-[420px] lg:h-[480px] bg-[#f3f3f4] relative group cursor-crosshair overflow-hidden rounded-xl border border-[#747878]/10">
            <img
              src={currentImage || '/images/touza_green_shirt.jpg'}
              alt={displayName}
              onError={(e) => {
                handleProductImageError(e, product.id);
              }}
            ...
            <div className="flex flex-wrap gap-3">
              {product.colors.map((c, cIdx) => {
                ...
                return (
                  <button
                    key={c.name}
                    onClick={() => handleSelectColor(c, cIdx)}
                    aria-label={colLabel}
                    title={colLabel}
