...
export const HeroBanner: React.FC<HeroBannerProps> = ({ onShopNow, storeSettings, onVideoReady }) => {
  ...
  const videoSrc = mediaUrl.startsWith('http')
    ? mediaUrl
    : mediaUrl.startsWith('/')
    ? mediaUrl
    : '/' + mediaUrl;
    ...
    <section className="relative h-[100dvh] min-h-[550px] sm:min-h-[600px] w-full flex items-end justify-center pb-10 sm:pb-16 md:pb-20 pt-24 sm:pt-28 px-4 overflow-hidden bg-[#0c0c0e]">
      ...
      {isVideo && (
        <video
          ...
          className={`hero-video absolute inset-0 w-full h-full object-cover object-center z-1 bg-transparent transition-opacity duration-700 ${
      ...
      {!isVideo && (
        <img
          ...
          alt={heroTitle}
          onError={(e) => {
            if (e.currentTarget.src !== RELIABLE_FALLBACK_IMAGES.hero) {
              e.currentTarget.src = RELIABLE_FALLBACK_IMAGES.hero;
            }
          }}
