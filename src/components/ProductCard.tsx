...
import { getFallbackForProduct, handleProductImageError } from '../utils/imageUtils';
...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const getAspectClass = () => {
    ...
    return 'h-[250px] sm:h-[270px] md:h-[290px] w-full';
  };

  const activeColor = product.colors?.[activeColorIndex];
  const mainProductImg = product.images?.[0];
  const rawImg = activeColorIndex === 0
    ? (mainProductImg || activeColor?.imageUrl)
    : (activeColor?.imageUrl || mainProductImg);
  const fallbackUrl = getFallbackForProduct(product.id, activeColor?.hex);
  const displayImage = rawImg && rawImg.trim() ? rawImg : fallbackUrl;
        ...
        >
          ...
          {displayImage && (
            <img
              ...
              onError={(e) => {
                setIsLoaded(true);
                handleProductImageError(e, product.id, activeColor?.hex);
              }}
              className="w-full h-full object-cover object-center transition-all duration-350 ease-out group-hover:scale-107 opacity-100"
              loading="eager"
              decoding="async"
