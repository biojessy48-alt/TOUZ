...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const showTopBar = storeSettings?.showAnnouncementBar !== false;
  const topAnnouncement = language === 'ar' ? storeSettings?.announcementAr : storeSettings?.announcementEn;
  const secondaryAnnouncement = language === 'ar' 
    ? (storeSettings?.announcementSecondaryAr || "توزا لأزياء الرجال - TOUZA MEN'S WEAR") 
    : (storeSettings?.announcementSecondaryEn || "TOUZA MEN'S WEAR - PREMIUM CASUALS");
  const customBg = storeSettings?.announcementBgColor;
  const isDefaultDark = !customBg || customBg === '#111111' || customBg === '#000000';
  const barBg = isTransparent && isDefaultDark ? 'rgba(0,0,0,0.45)' : (customBg || '#111111');
  const barText = storeSettings?.announcementTextColor || '#e2c792';
  const animSpeed = storeSettings?.announcementSpeed === 'fast' ? '14s' : storeSettings?.announcementSpeed === 'slow' ? '36s' : '24s';
  ...
  return (
    <>
      ...
      >
        ...
        {showTopBar && (topAnnouncement || secondaryAnnouncement) && (
          <div 
            className="w-full text-[11px] sm:text-[12px] font-medium py-1.5 border-b border-black/10 overflow-hidden font-label-caps whitespace-nowrap marquee-container select-none transition-colors duration-300"
              ...
              <div className="flex items-center gap-12 shrink-0">
                {topAnnouncement && (
                  <span className="flex items-center gap-3">
                    <span style={{ color: barText }} className="text-[10px] opacity-80">✦</span>
                ...
                {secondaryAnnouncement && (
                  <span className="flex items-center gap-3">
                    <span style={{ color: barText }} className="text-[10px] opacity-80">✦</span>
                    <span className="tracking-[0.18em]">{secondaryAnnouncement}</span>
                  </span>
                )}
                {topAnnouncement && (
                  <span className="flex items-center gap-3">
