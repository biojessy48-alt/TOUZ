...
export const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('maison_categories', JSON.stringify(categories));
    } catch (err) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('maison_promos', JSON.stringify(promoCodes));
    } catch (err) {
  ...
  const defaultSettings: StoreSettings = {
    ...
    announcementSecondaryAr: "توزا لأزياء الرجال - TOUZA MEN'S WEAR",
    announcementSecondaryEn: "TOUZA MEN'S WEAR - PREMIUM CASUALS",
    announcementBgColor: '#111111',
    announcementTextColor: '#e2c792',
  ...
  useEffect(() => {
    try {
      localStorage.setItem('maison_settings', JSON.stringify(storeSettings));
    } catch (err) {
  ...
  const handleDeleteProduct = async (productId: string) => {
    setProducts((prev) => {
      ...
      localStorage.setItem('maison_products', JSON.stringify(newList));
      ...
      <main className="flex-1">
        ...
        {currentView === 'home' && (
          <div>
            ...
            <ScrollReveal>
              <section id="featured-products" className="pt-28 pb-16 md:pt-36 md:pb-20 px-5 md:px-16 max-w-[1440px] mx-auto w-full scroll-mt-32">
                ...
                {(() => {
                  const listToDisplay = products && products.length > 0 ? products : PRODUCTS;
